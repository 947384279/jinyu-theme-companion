...
function jinyu_seo_meta()
{
    ...
    if (!is_404()) {
        $canonical = '';
        if (is_singular()) {
            $canonical = get_permalink() . $wx_suffix;
        } elseif (is_front_page() || is_home()) {
            $canonical = home_url('/') . $wx_suffix;
    ...
    if (is_singular()) {
        ...
        if ($og_image === $og_img && $og_img) {
            ...
            $d = jinyu_og_image_dims($og_image);
            if ($d) {
                $og_w = $d[0];
                $og_h = $d[1];
            }
        ...
        echo '<meta property="og:url" content="' . esc_url(get_permalink() . $wx_suffix) . '">' . PHP_EOL;
        ...
        if (jinyu_is_checked('twitter_card_enable')) {
            ...
            if ($og_image) echo '<meta name="twitter:image" content="' . esc_url($og_image) . '">' . PHP_EOL;
        }
    } elseif (is_front_page() || is_home()) {
        ...
        if ($home_img === jinyu_get_option('og_image', '') && $home_img) {
            ...
            $d = jinyu_og_image_dims($home_img);
            if ($d) {
                $og_w = $d[0];
                $og_h = $d[1];
            }
        ...
        echo '<meta property="og:url" content="' . esc_url(home_url('/') . $wx_suffix) . '">' . PHP_EOL;
        echo '<meta property="og:site_name" content="' . esc_attr($site_name) . '">' . PHP_EOL;
        echo '<meta property="og:locale" content="' . str_replace( '_', '-', get_locale() ) . '">' . PHP_EOL;
...
function jinyu_archive_title_parts($parts)
{
    ...
    if (is_category() || is_tag() || is_tax() || is_author() || is_date() || is_post_type_archive()) {
        if (is_category()) {
            $parts['title'] = sprintf(__('%s 分类', 'jinyu-theme-companion'), single_term_title('', false));
        } elseif (is_tag()) {
            $parts['title'] = sprintf(__('%s 标签', 'jinyu-theme-companion'), single_term_title('', false));
        } elseif (is_tax()) {
