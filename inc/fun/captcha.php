...
function jinyu_captcha_verify(string $scene, string $input)
{
    ...
    return new WP_Error('captcha_error', __('验证码不正确或已过期', 'jinyu-theme-companion'));
}
...
function jinyu_captcha_svg(string $code): string
{
    ...
    // 字符：旋转 + 斜切 + 垂直抖动 + 字号微扰，提升机器识别难度（校验契约不变）
    $len = strlen($code);
    for ($i = 0; $i < $len; $i++) {
        $ch   = $code[$i];
        $x    = 14 + $i * 24;
        ...
        $parts[] = sprintf(
            '<text x="%d" y="%d" font-family="Menlo,Consolas,monospace" font-size="%d" font-weight="700" fill="%s" '
            . 'transform="rotate(%d %d %d) skewX(%d) translate(0 %d)">%s</text>',
            $x,
            $y,
            $fs,
            $color,
