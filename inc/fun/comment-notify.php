...
function jinyu_comment_notify($comment_id, $comment_approved)
{
    ...
    $subject = sprintf('[%s] %s', get_bloginfo('name'), __('有新的评论回复', 'jinyu-theme-companion'));
    ...
    if ($comment->comment_parent > 0) {
        ...
        if ($parent && $parent->comment_author_email && $parent->comment_author_email !== $comment->comment_author_email) {
            ...
            $body = '<p>' . sprintf(__('你在《%s》下的评论有了新回复', 'jinyu-theme-companion'), $post->post_title) . '</p>'
                   ...
                   . '<p><a href="' . esc_url($link) . '">' . __('查看回复', 'jinyu-theme-companion') . '</a></p>';
    ...
    if ($post_author_email && $post_author_email !== $comment->comment_author_email) {
        ...
        $body = '<p>' . sprintf(__('你的文章《%s》有新评论', 'jinyu-theme-companion'), $post->post_title) . '</p>'
               . '<p><strong>' . esc_html($comment->comment_author) . '：</strong>' . esc_html($comment->comment_content) . '</p>'
               . '<p><a href="' . esc_url($link) . '">' . __('查看文章', 'jinyu-theme-companion') . '</a></p>';
